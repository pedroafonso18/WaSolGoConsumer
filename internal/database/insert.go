...
func UpsertChat(db *sql.DB, chat *parser.Chat) error {
	if chat.Tabulation == nil {
		query := "INSERT INTO chats (id, situation, is_active, agent_id, customer_id) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (id) DO UPDATE SET situation = $2, is_active = $3, agent_id = $4, customer_id = $5"
		_, err := db.Exec(query, chat.ID, chat.Situation, chat.IsActive, chat.AgentID, chat.CustomerID)
		if err != nil {
			return fmt.Errorf("couldn't insert chat into database: %w", err)
		}
	} else {
		query := "INSERT INTO chats (id, situation, is_active, agent_id, tabulation, customer_id) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO UPDATE SET situation = $2, is_active = $3, agent_id = $4, tabulation = $5, customer_id = $6"
		_, err := db.Exec(query, chat.ID, chat.Situation, chat.IsActive, chat.AgentID, chat.Tabulation, chat.CustomerID)
		if err != nil {
			return fmt.Errorf("couldn't insert chat into database: %w", err)
		}
	}
